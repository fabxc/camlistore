...
// parse such headers.
func NewMultipartReader(r *bufio.Reader, boundary string) *MultipartReader {
	b := []byte("\r\n--" + boundary + "--")
...
func (bp *Part) populateHeaders() error {
	hbts, err := readUntilBody(bp.mr.bufReader)
	if err != nil {
		return err
	}
	bp.headerBytes = hbts
	header, err := readMIMEHeader(hbts)
	if err == nil {
		bp.Header = header
	}
	return err
...
func (r *MultipartReader) LastDelimLine() []byte {
	b := r.lastDelimLine
	r.lastDelimLine = nil
	return b
}
...
func (r *MultipartReader) NextPart() (*Part, error) {
	...
	for {
		...
		if err == io.EOF && r.isFinalBoundary(line) {
			r.lastDelimLine = append(r.lastDelimLine, line...)
...
func (mr *MultipartReader) peekBufferIsEmptyPart(peek []byte) bool {
	// End of parts case.
